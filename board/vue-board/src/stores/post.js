...
export const usePostStore = defineStore('post', () => {
  ...
  const replies = ref([]); // 현재 게시글 댓글 목록
  ...
  const updatePost = async (data) => {
    ...
    if (!authStore.hasPermission(postToUpdate.writerId)) {
  ...
  const deletePost = async (id, password = null) => {
    ...
    if (!isGuest && !authStore.hasPermission(postToDelete.writerId)) {
      throw new Error('AUTH_ERROR: 삭제 권한이 없습니다.');
    }
    if (isGuest && !password) {
      throw new Error('PASSWORD_REQUIRED: 게스트는 비밀번호 필요');
    }
